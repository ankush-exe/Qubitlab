...
export default function App() {
  ...
  const [mode, setMode] = useState<"designer" | "visual">("designer");
  ...
  return <div className="app-shell">
    <header className="topbar"><div className="brand"><span className="brand-mark">Q</span><div><strong>QubitLab</strong><span>quantum learning studio</span></div></div><nav><button onClick={() => setMode("designer")} className={mode === "designer" ? "nav-active" : ""} type="button">Circuit Designer</button><button onClick={() => setMode("visual")} className={mode === "visual" ? "nav-active" : ""} type="button">Visual Coding</button><button onClick={() => { setMode("visual"); window.setTimeout(() => document.querySelector(".code-panel")?.scrollIntoView({ behavior: "smooth" }), 0); }} type="button">Python Code</button><button type="button" title="Learning modules are coming soon">Learn <span>soon</span></button><button type="button" title="AI Tutor is coming soon">AI Tutor <span>soon</span></button><button type="button" title="Quiz is coming soon">Quiz <span>soon</span></button><button type="button" title="Progress tracking is coming soon">Progress <span>soon</span></button></nav><div className="runtime"><i /> local runtime <b>v0.2</b></div></header>
    {mode === "designer" ? <main className="workspace" id="circuit">
    ...
    </main> : <VisualCoding onShowCircuit={(visualCircuit) => { setCircuit(visualCircuit); setResult(null); setError(null); setMode("designer"); }} />}
